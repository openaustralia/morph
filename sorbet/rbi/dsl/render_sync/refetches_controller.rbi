# typed: true

# DO NOT EDIT MANUALLY
# This is an autogenerated file for dynamic methods in `RenderSync::RefetchesController`.
# Please instead update this file by running `bin/tapioca dsl RenderSync::RefetchesController`.

class RenderSync::RefetchesController
  sig { returns(HelperProxy) }
  def helpers; end

  module HelperMethods
    include ::Ransack::Helpers::FormHelper
    include ::ApplicationHelper
    include ::ApiHelper
    include ::BootstrapFlashHelper
    include ::ConnectionLogsHelper
    include ::DocumentationHelper
    include ::OwnersHelper
    include ::RunsHelper
    include ::ScrapersHelper
    include ::SearchHelper
    include ::SiteSettingsHelper
    include ::StaticHelper
    include ::SupportersHelper
    include ::UsersHelper
    include ::RenderSync::ConfigHelper
    include ::FontAwesome::Rails::IconHelper
    include ::DeviseHelper
  end

  class HelperProxy < ::ActionView::Base
    include HelperMethods
  end
end
