  ...
  describe "differencing databases" do
    ...
    describe ".diffstat_table" do
      it "should show no change for two identical sqlite databases" do
        expect(Morph::Database.diffstat_table("foo", @db1, @db2)).to eq ({added: 0, removed: 0, changed: 0, unchanged: 1})
      end

      it "should show a new record" do
        @db2.execute("INSERT INTO foo VALUES ('goodbye', 3.1)")
        expect(Morph::Database.diffstat_table("foo", @db1, @db2)).to eq ({added: 1, removed: 0, changed: 0, unchanged: 1})
      end

      it "should show a deleted record" do
        @db2.execute("DELETE FROM foo where v1='hello'")
        expect(Morph::Database.diffstat_table("foo", @db1, @db2)).to eq ({added: 0, removed: 1, changed: 0, unchanged: 0})
      end

      it "should show adding a record and deleting a record" do
        @db2.execute("INSERT INTO foo VALUES ('goodbye', 3.1)")
        @db2.execute("DELETE FROM foo where v1='hello'")
        expect(Morph::Database.diffstat_table("foo", @db1, @db2)).to eq ({added: 1, removed: 1, changed: 0, unchanged: 0})
      end

      it "should show a record being changed" do
        @db2.execute("UPDATE foo SET v1='different' WHERE v1='hello'")
        expect(Morph::Database.diffstat_table("foo", @db1, @db2)).to eq ({added: 0, removed: 0, changed: 1, unchanged: 0})
      end
      ...
      it "should be able to handle a large number of records", slow: true do
        ...
        expect(Morph::Database.diffstat_table("foo", @db1, @db2, 100)).to eq ({added: 200, removed: 200, changed: 100, unchanged: 700})
      end
      ...
      it "should compare two empty dbs" do
        ...
        FileUtils::cp("tmp_db1.sqlite", "tmp_db2.sqlite")
        @db2 = SQLite3::Database.new("tmp_db2.sqlite")
        expect(Morph::Database.diffstat_table("foo", @db1, @db2)).to eq ({added: 0, removed: 0, changed: 0, unchanged: 0})
      end
    end

    describe ".diffstat" do
      it "should show that nothing has changed" do
        expect(Morph::Database.diffstat("tmp_db1.sqlite", "tmp_db2.sqlite")).to eq ({
          tables: {
            added: [],
            removed: [],
            changed: [],
      ...
      it "should show everything as added when there was no database to start with" do
        expect(Morph::Database.diffstat("non_existent_file.sqlite", "tmp_db2.sqlite")).to eq ({
          tables: {
            added: [
              {
                name: "foo",
      ...
      it "should show no difference when comparing two non-existent databases" do
        expect(Morph::Database.diffstat("non_existent_file1.sqlite", "non_existent_file2.sqlite")).to eq ({
          tables: {
            added: [],
            removed: [],
            changed: [],
