...
describe RunsHelper do
  describe "#scraped_domains_list" do
    ...
    it do
      allow(run).to receive(:scraped_domain_objects).and_return([foo_domain])
    ...
    it do
      allow(run).to receive(:scraped_domain_objects).and_return([foo_domain, bar_domain])
      expect(helper.scraped_domains_list(run)).to eq '<a href="http://foo.com">foo.com</a> and <a href="http://bar.com">bar.com</a>'
      expect(helper.scraped_domains_list(run)).to be_html_safe
    end

    it do
      allow(run).to receive(:scraped_domain_objects).and_return([foo_domain, bar_domain, www_foo_domain])
    ...
    it do
      allow(run).to receive(:scraped_domain_objects).and_return([foo_domain, bar_domain, www_foo_domain, www_bar_domain])
    ...
    it do
      allow(run).to receive(:scraped_domain_objects).and_return([foo_domain, bar_domain, www_foo_domain, www_bar_domain, fiddle_domain])
