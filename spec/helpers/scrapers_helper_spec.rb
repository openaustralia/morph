...
describe ScrapersHelper do
  describe "#url?" do
    it { expect(helper.url?("foobar")).to eq false }
    it { expect(helper.url?("http://example.com blah")).to eq false }
    it { expect(helper.url?("ftp://example.com/no_ftp")).to eq false }
    it { expect(helper.url?('<a href="http://example.com">blah</a>')).to eq false }
    it { expect(helper.url?("http://example.com")).to eq true }
    it { expect(helper.url?("http://example.com/#anchor")).to eq true }
  end
